...
async function createErr(coin, vsCoin='usd') {
  ...
  const msgPayload = { title: "Coin not found!",  thumbnail: exchIcon, header: `${coinProper} vs ${vsCoinProper}`, content: `**${errorCoin}** does not seem to be valid.`, source: 'coingecko' };
  return await createMsg(msgPayload);
}
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let cgData = await mulaRDB.get('cgscoins');
    if (!cgData) {
      cgData = (await CoinGeckoClient.coins.list()).data
      await mulaRDB.set('cgscoins', cgData, 604800000);
    }

    let coinMatch, coinData, coinVsUsd, coinToUsd, vsCoinMatch, vsCoinData, vsCoinVsUsd, coinVsCoin, msgPayload, embed;
    coinMatch = cgData.filter(coins => coins.symbol === coin && (coins.name.split(" ").length) < 2 );
    ...
    if (vsCoin === '') {
      msgPayload = { title: "Crypto Convert", thumbnail: exchIcon, header: `${coinProper} vs USD`, content: `**${amount} ${coinProper}** to USD is : **${formatCurrency(amount * coinVsUsd, "USD", "en")}**`, source: 'coingecko' };
      embed = await createMsg(msgPayload);
      await interaction.editReply({ embeds: [embed] });
    }

    else {
      coinToUsd = amount * coinVsUsd;
      vsCoinMatch = await cgData.filter(coins => coins.symbol === vsCoin && (coins.name.split(" ").length) < 2);
      ...
      coinVsCoin = coinToUsd / vsCoinVsUsd;
      msgPayload = { title: "Crypto Convert", thumbnail: exchIcon, header: `${coinProper} vs ${vsCoinProper}`, content: `**${amount} ${coinProper}** to ${vsCoinProper} is : **${coinVsCoin.toFixed(5)} ${vsCoinProper}**`, source: 'coingecko' }
      embed = await createMsg(msgPayload);
      await interaction.editReply({ embeds: [embed] });
    }
